...
function commitServerChanges(serverData, objectCache, changes) {
  ...
  for (var _attr3 in changes) {
    ...
    if (val && typeof val === 'object' && !Array.isArray(val) && Object.keys(val).length > 0 && Object.keys(val).some(function (k) {
      return !isNaN(parseInt(k));
    }) && !['sentPerUTCOffset', 'failedPerUTCOffset'].includes(_attr3)) {
